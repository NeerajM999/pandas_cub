...
class DataFrame:
    ...
    def _check_input_types(self, data):
        ...
        for key, value in data.items():
            ...
            if value.ndim != 1:
                raise ValueError('values of data must be one dimensional array')
    ...
    def columns(self):
        ...
        """
        return list(self._data) # no need to add .keys() as list will only emit keys of the dict
    ...
    def columns(self, columns):
        ...
        if len(columns) != len(self._data):
            raise ValueError('`columns` size should match data size')
        for c in columns:
            if not isinstance(c, str):
                raise TypeError('columns should be string type')
        if len(columns) != len(set(columns)):
            raise ValueError('columns should not have duplicate names')
    ...
    def dtypes(self):
        ...
        new_data = {'Column Name': np.array(list(self._data.keys())), 'Data Type': dtypes}
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, DataFrame):
            ...
            print(f"arr:{arr}")
            if arr.dtype.kind != 'b':
                raise ValueError('item must be one column boolean DataFrame')
        ...
        raise TypeError('You must pass either a str, list, dataframe or tuple to the selection operator')
    ...
    def _getitem_tuple(self, item):
        ...
        if isinstance(row_selection, int):
            ...
            raise TypeError('row selection must be int, list, slice or DataFrame')
        ...
        if isinstance(col_selection, int):
            ...
            raise TypeError('col selection must be int, str, slice or dataframe')
    ...
    def __setitem__(self, key, value):
        ...
        if not isinstance(key, str):
            raise NotImplementedError('Dataframe can only be set a single col')

        if isinstance(value, np.ndarray):
            if value.ndim != 1:
                raise ValueError('Numpy error must be one dimensional')
            if len(value) != len(self):
                raise ValueError('Length mismatch')
        elif isinstance(value, DataFrame):
            if value.shape[1] != 1:
                raise ValueError('The setting dataframe must be one column')
            if len(value) != len(self):
            ...
            raise TypeError('setting object must be array, int, float, str, dataframe')
        if value.dtype.kind == 'U':
    ...
    def rename(self, columns):
        ...
        if not isinstance(columns, dict):
            raise TypeError('columns parameter must be a dictionary type')

        new_data = {}
        for col, value in self._data.items():
            # if col in columns.keys():
            #     new_data[columns[col]] = value
            # else:
            #     new_data[col] = value
    ...
    def drop(self, columns):
        ...
        if isinstance(columns, str):
            ...
            raise TypeError('columns must be either str or list type')
    ...
    def sort_values(self, by, asc=True):
        ...
        if isinstance(by, str):
            ...
            by = [self._data[col] for col in by[::-1]] # reverse 'by' for lexsort
            order = np.lexsort(by)
        else:
    ...
    def pivot_table(self, rows=None, columns=None, values=None, aggfunc=None):
        ...
        for group, vals in d.items():
            arr = np.array(vals)
            func = getattr(np, aggfunc) # returns np.sum or np.mean, etc from np module
        ...
        if pivot_type == 'columns':
            ...
            rows_vals = np.array(list(agg_dict.keys()))
            vals = np.array(list(agg_dict.values()))
            print(f"rows_vals: {rows_vals}")
            print(f"vals: {vals}")

            order = np.argsort(rows_vals)
            new_data[rows] = rows_vals[order]
            ...
            for col in col_list:
                ...
                print(f"row: {row_list}, col: {col_list}, new_vals: {new_vals}")
...
class StringMethods:
    ...
    def _str_method(self, method, col, *args):
        ...
        if value.dtype.kind != 'O':
            raise TypeError('`str` accessor only works with string')
        new_vals = []
        for val in value:
            if val is None:
